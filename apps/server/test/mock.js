const https = require('https'),
    ...
    MOCK_SERVER_URL =
        ...
        `http://${MOCK_SERVER_DOMAIN}:${MOCK_SERVER_PORT}`,
...
async function restController(req, res) {
    ...
    if (this.routes[method] && this.routes[method][path]) {
        ...
        if (
            300 <= this.routes[method][path].status &&
            400 > this.routes[method][path].status
        ) {
            let location =
                typeof responseBody === 'function'
                    ? await responseBody(req)
                    : responseBody;
            if (0 < Object.keys(req.query).length) {
                location += '?' + querystring.stringify(req.query);
            }
            res.redirect(this.routes[method][path].status, location);
        } else {
            res.status(this.routes[method][path].status).send(
                typeof responseBody === 'function'
                    ? await responseBody(req)
                    : responseBody
            );
        ...
        res.status(501).send(`Unknown route ${method} ${path}`);
    }
}
...
async function rpcController(req, res) {
    try {
        ...
        if (this.routes.RPC2[method]) {
            ...
            res.status(501).send(
                rpcReturnFault(1, `Unknown methodName ${method}`)
            );
        }
    } catch (err) {
        res.status(500).send(rpcReturnFault(1, err.message));
    }
}
...
module.exports = {
    ...
    before: async function () {
        ...
        this.secureServer = await https
            ...
            .listen(SECURE_MOCK_SERVER_PORT);
        console.log(
            `    → Mock secure server started on port: ${SECURE_MOCK_SERVER_PORT}`
        );
    },
