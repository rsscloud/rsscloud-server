...
async function checkPingFrequency(resource) {
    let ctsecs,
        minsecs = config.minSecsBetweenPings;
    if (0 < minsecs) {
        const dayjs = await getDayjs();
        ctsecs = dayjs().diff(resource.whenLastCheck, 'seconds');
        if (ctsecs < minsecs) {
            throw new ErrorResponse(
                appMessage.error.ping.tooRecent(minsecs, ctsecs),
                'PING_TOO_RECENT'
            );
        }
    }
...
async function checkForResourceChange(resource, resourceUrl, startticks) {
    ...
    if (res.status < 200 || res.status > 299) {
        throw new ErrorResponse(
            appMessage.error.ping.readResource(resourceUrl)
        );
    }
...
async function ping(resourceUrl) {
    ...
    const startticks = dayjs().format('x'),
        resource = await initResource(await fetchResource(resourceUrl));

    await checkPingFrequency(resource);
    const changed = await checkForResourceChange(
        resource,
        resourceUrl,
        startticks
    );
