...
function rest(req) {
    ...
    if (0 === s.length) {
        parts.scheme =
            'https-post' === params.protocol || '443' === String(req.body.port)
                ? 'https'
                : 'http';
        ...
        throw new ErrorResponse(
            appMessages.error.subscription.missingParams(s)
        );
    }
}
...
function rpc(req, rpcParams) {
    ...
    if (5 > rpcParams.length) {
        throw new ErrorResponse(
            appMessages.error.rpc.notEnoughParams('pleaseNotify')
        );
    } else if (6 < rpcParams.length) {
        throw new ErrorResponse(
            appMessages.error.rpc.tooManyParams('pleaseNotify')
        );
    }
    ...
    parts.scheme =
        'https-post' === params.protocol || '443' === String(rpcParams[1])
            ? 'https'
            : 'http';
